...
async function filterCategory(){   
    ...
    // Send ISO strings or empty
    jsonObj['createdFromDate'] = createdFromDateElem.value 
        ? new Date(createdFromDateElem.value).toISOString() 
        : "";
    jsonObj['createdToDate'] = createdToDateElem.value 
        ? new Date(createdToDateElem.value).toISOString() 
        : "";
    ...
    try {
        ...
        await populateCategoryListVResponse(data,"category_list_form");
    } catch (error) {
        handleErrorForList(itemName,error,'category_list_table_container');
    }
};
