...
CREATE OR REPLACE FUNCTION public.getFilterUsersDetailsList(
														    userId VARCHAR,
														    userName VARCHAR,    
														    mobileNo VARCHAR,  
														    roleName VARCHAR,
	...
	BEGIN
	    ...
	    WHERE m1.user_id IS NOT NULL
	      AND (userId = '' OR lower(m1.user_id) ILIKE lower(userId) || '%')
	      AND (userName = '' OR lower(m1.user_name) ILIKE lower(userName) || '%')
	      AND (mobileNo = '' OR lower(m1.mobile_no) ILIKE lower(mobileNo) || '%')
	      AND (roleName = '' OR lower(m1.role_name) ILIKE lower(roleName) || '%')
	      AND (branchName = '' OR lower(m1.branch) ILIKE lower(branchName) || '%')
	      AND (repCode = '' OR lower(m1.rep_code) ILIKE lower(repCode) || '%')
	      AND (repAccount = '' OR lower(m1.rep_account) ILIKE lower(repAccount) || '%')
	      AND (leaderUserId = '' OR lower(m1.leader_user_id) ILIKE lower(leaderUserId) || '%');
	END;
$BODY$;
...
SELECT * FROM getFilterCategoryDetailsList('CTY_1','','','','');

CREATE OR REPLACE FUNCTION public.getFilterCategoryDetailsList(
														    categoryId VARCHAR,
														    categoryName VARCHAR,    
														    createdFromDate VARCHAR,
														    createdToDate VARCHAR,
														    createdBy VARCHAR
														)
	...
	BEGIN
	    ...
	    FROM
	        product_category m1
	    WHERE     
	        (categoryId IS NULL OR categoryId = '' OR lower(m1.category_id) ILIKE lower(categoryId) || '%')            
	        AND (categoryName IS NULL OR categoryName = '' OR lower(m1.category) ILIKE lower(categoryName) || '%')        
	        AND (
	            createdFromDate IS NULL OR createdFromDate = '' 
	            OR createdToDate IS NULL OR createdToDate = '' 
	            OR m1.created_date BETWEEN createdFromDate::timestamp AND createdToDate::timestamp
	        )
	        AND (createdBy IS NULL OR createdBy = '' OR lower(m1.created_by) ILIKE lower(createdBy) || '%')
	    ORDER BY
	        m1.created_date DESC;
	END;
$BODY$;
...
CREATE OR REPLACE FUNCTION public.getFilterProductDetailsList(
													    ...
													    createdFromDate VARCHAR,  -- changed from TIMESTAMP
													    createdToDate VARCHAR,    -- changed from TIMESTAMP
													    createdBy VARCHAR
													)
		...
		BEGIN
		    ...
		    FROM
		        product_details m1
		    WHERE
		        (productId IS NULL OR productId = '' OR lower(m1.product_id) ILIKE lower(productId) || '%')                 
		        AND (productName IS NULL OR productName = '' OR lower(m1.product_name) ILIKE lower(productName) || '%')      
		        AND (productCategory IS NULL OR productCategory = '' OR lower(m1.product_category) ILIKE lower(productCategory) || '%')        
		        AND (
		            (createdFromDate IS NULL OR createdFromDate = '' OR createdToDate IS NULL OR createdToDate = '')
		            OR m1.created_date BETWEEN createdFromDate::timestamp AND createdToDate::timestamp
		        )
		        AND (createdBy IS NULL OR createdBy = '' OR lower(m1.created_by) ILIKE lower(createdBy) || '%')
		    ORDER BY
...
CREATE OR REPLACE FUNCTION public.getFilterOrderDetailsList(
													    ...
													    ddNo VARCHAR,
													    customerName VARCHAR,
													    orderPerformFromDate VARCHAR,   -- was TIMESTAMP
													    orderPerformToDate VARCHAR,     -- was TIMESTAMP
													    repAttD VARCHAR,                                                                                                                
													    mobileNo VARCHAR,
													    demoPlan VARCHAR,
													    demoFromDate VARCHAR,           -- was TIMESTAMP
													    demoToDate VARCHAR,             -- was TIMESTAMP
													    itsHaveDiscount VARCHAR,
													    orderStatus VARCHAR,
													    createdFromDate VARCHAR,        -- was TIMESTAMP
													    createdToDate VARCHAR,          -- was TIMESTAMP
													    createdBy VARCHAR
													)
													RETURNS SETOF order_details
													LANGUAGE plpgsql
	...
	BEGIN
	    RETURN QUERY
	    SELECT m1.*
	    FROM order_details m1
	    WHERE
	        (eNo IS NULL OR eNo = '' OR lower(m1.e_no) ILIKE lower(eNo) || '%') 
	        AND (estNo IS NULL OR estNo = '' OR lower(m1.est_no) ILIKE lower(estNo) || '%') 
	        AND (orderNo IS NULL OR orderNo = '' OR lower(m1.order_no) ILIKE lower(orderNo) || '%')  
	        AND (soNo IS NULL OR soNo = '' OR lower(m1.so_no) ILIKE lower(soNo) || '%')  
	        AND (ddNo IS NULL OR ddNo = '' OR lower(m1.d_d_no) ILIKE lower(ddNo) || '%')  
	        AND (customerName IS NULL OR customerName = '' OR lower(m1.customer_name) ILIKE lower(customerName) || '%')        
	        AND (
	            orderPerformFromDate IS NULL OR orderPerformFromDate = '' 
	            OR orderPerformToDate IS NULL OR orderPerformToDate = '' 
	            OR m1.order_process_date BETWEEN orderPerformFromDate::timestamp AND orderPerformToDate::timestamp
	        )                           
	        AND (repAttD IS NULL OR repAttD = '' OR lower(m1.rep_code) ILIKE lower(repAttD) || '%')                                 
	        AND (mobileNo IS NULL OR mobileNo = '' OR lower(m1.customer_phone) ILIKE lower(mobileNo) || '%')
	        AND (demoPlan IS NULL OR demoPlan = '' OR lower(m1.demo_plan) ILIKE lower(demoPlan) || '%')        
	        AND (
	            demoFromDate IS NULL OR demoFromDate = '' 
	            OR demoToDate IS NULL OR demoToDate = '' 
	            OR m1.demo_date BETWEEN demoFromDate::timestamp AND demoToDate::timestamp
	        )            
	        AND (itsHaveDiscount IS NULL OR itsHaveDiscount = '' OR lower(m1.its_have_discount) ILIKE lower(itsHaveDiscount) || '%')                     
	        AND (orderStatus IS NULL OR orderStatus = '' OR lower(m1.register_status) ILIKE lower(orderStatus) || '%')                              
	        AND (
	            createdFromDate IS NULL OR createdFromDate = '' 
	            OR createdToDate IS NULL OR createdToDate = '' 
	            OR m1.created_date BETWEEN createdFromDate::timestamp AND createdToDate::timestamp
...
DROP FUNCTION IF EXISTS getFSMUserIdsBasedDarDetailsList(VARCHAR,TIMESTAMP,TIMESTAMP,VARCHAR,VARCHAR,TIMESTAMP,TIMESTAMP,VARCHAR) CASCADE;
 CREATE OR REPLACE FUNCTION public.getFilterDarDetailsList(
														    darNo VARCHAR,
														    darPerformFromDate VARCHAR,  -- changed from TIMESTAMP
														    darPerformToDate VARCHAR,    -- changed from TIMESTAMP
														    clientName VARCHAR,
	...
	BEGIN
	    ...
	    FROM
	        dar_details m1
	    WHERE
	        (darNo IS NULL OR darNo = '' OR lower(m1.dar_no) ILIKE lower(darNo) || '%')
	        AND (
	            darPerformFromDate IS NULL OR darPerformFromDate = '' 
	            OR darPerformToDate IS NULL OR darPerformToDate = '' 
	            OR m1.dar_process_date BETWEEN darPerformFromDate::timestamp AND darPerformToDate::timestamp
	        )
	        AND (clientName IS NULL OR clientName = '' OR lower(m1.client_name) ILIKE lower(clientName) || '%')
	        AND (clientMobileNo IS NULL OR clientMobileNo = '' OR lower(m1.client_mobile_no) ILIKE lower(clientMobileNo) || '%')
	        AND (statusToVisit IS NULL OR statusToVisit = '' OR lower(m1.status_to_visit) ILIKE lower(statusToVisit) || '%')
	        AND (
	            createdFromDate IS NULL OR createdFromDate = '' 
	            OR createdToDate IS NULL OR createdToDate = '' 
	            OR m1.created_date BETWEEN createdFromDate::timestamp AND createdToDate::timestamp
...
CREATE OR REPLACE FUNCTION public.getFilterPaymentsDetailsList(
	...
	BEGIN
	    ...
	    WHERE
	        ...
	        AND (itsHaveDiscount IS NULL OR itsHaveDiscount = '' OR lower(m1.its_have_discount) ILIKE lower(itsHaveDiscount) || '%')                     
	        AND (paymentStatus IS NULL OR paymentStatus = '' OR lower(m1.register_status) ILIKE lower(paymentStatus) || '%')                              
	        AND (
	            createdFromDate IS NULL OR createdFromDate = '' 
	            OR createdToDate IS NULL OR createdToDate = '' 
	            OR m1.created_date BETWEEN createdFromDate::timestamp AND createdToDate::timestamp
